...
const details = {scanner:['🐱','อ่านตลาดและคัดกรองคู่เทรด'],strategy:['🐰','วิเคราะห์สัญญาณจากกลยุทธ์'],risk:['🐻','ตรวจขนาดสถานะและ risk lock'],execution:['🐶','จำลองการเข้าออกสถานะ'],news:['🦉','ยังไม่ได้เชื่อมแหล่งข่าว'],testnet:['🐼','อ่านสถานะที่ Testnet pilot บันทึกไว้']};
...
function render(data){
 ...
 const rows=[['Paper service',labels[data.services?.['gptsalov-paper.service']?.status]||'—'],['Forward service',labels[data.services?.['gptsalov-forward.service']?.status]||'—'],['โหมดที่อ่านได้',l.mode==='paper'?'Paper simulator':'ยังไม่มีข้อมูล'],['อัปเดตจากบอท',date(l.observed_at_ms)],['ข้อมูลแท่งราคา',date(l.as_of_ms)],['สถานะจำลองที่ถือ',l.position_symbol||'—'],['Risk lock',l.locked===undefined?'—':l.locked?'ล็อกอยู่':'ไม่ล็อก'],['Tuning service',labels[data.services?.['gptsalov-tuning.service']?.status]||'—'],['Shadow service',labels[data.services?.['gptsalov-multiagent-shadow.service']?.status]||'—'],['Testnet service',labels[data.services?.['gptsalov-testnet-pilot.service']?.status]||'—'],['Testnet ledger',labels[data.testnet?.status]||'—'],['Testnet phase',data.testnet?.phase||'—'],['Testnet อัปเดต',date(data.testnet?.checked_at_ms)],['News / AI','ยังไม่เชื่อม']];
