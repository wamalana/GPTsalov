...
VARIANTS = {
    ...
    # v3.1 (round 2, chosen on in-sample of the predeclared grid): stop 1.0 ATR and
    # two slots limited to one per direction. See STRATEGY_V2.md section 6.
    "v3_1_4h_trail":    (16, "v2_s1", replace(V1, bar_ms=4*HOUR_MS, max_hold_bars=42, reanchor=True,
                                            max_adverse_drift_r=0.25, cost_gate=0.25, exit_mode="trail",
                                            trail_atr=3.0, max_positions=2, one_per_side=True)),
...
def run_variant(name, data15: dict, funding=None, cost_mult=1.0, v2_params=V2Params()):
    ...
    if kind == "v1":
        ...
        btc = data.get("BTCUSDT")
        params = (replace(v2_params, target_r=3.0) if kind == "v2_3r"
                  else replace(v2_params, stop_atr=1.0) if kind == "v2_s1" else v2_params)
        sigs = {s: v2_signals(x, params, btc) for s, x in data.items()}
    res = simulate(data, sigs, eng, funding)
    res.signal_count = sum(len(x) for x in sigs.values())
    return res
